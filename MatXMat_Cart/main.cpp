...
int main(int argc, char **argv) {
	...
	double t_read_write_start = 0.0, t_read_write_end = 0.0;
	t_read_write_start = MPI_Wtime();
	if ( coord[2] == 0){
		...
		int offset = sizeof(double) *  aux6 +  2 * sizeof(int);
		for (int i = 0; i < length_block; i++) {
			int block =  sizeof(double) * ((i / N) * lenght + i % N);
			MPI_File_write_at(fh, offset + block, &aux4[i], 1, MPI_DOUBLE,&status);
		}
		
	}
	t_read_write_end = MPI_Wtime() - t_read_write_start;
	
	double t_read_write_total;
	if (my_grid_rank != 0) {
		MPI_Send(&t_read_write_end, 1, MPI_DOUBLE, 0, rank, grid_comm);
	}

	if (my_grid_rank == 0) {
		t_read_write_total += t_read_write_end;
		for (int i = 0; i < my_size - 1; ++i) {
			MPI_Recv(&t_read_write_end, 1, MPI_DOUBLE, MPI_ANY_SOURCE, MPI_ANY_TAG, grid_comm,&status);
			t_read_write_total += t_read_write_end;
		}
	}	
	...
	if (my_grid_rank == 0) {
		...
		if (my_size == 1) {
			...
			ofstream fil_time("time_read_write", ios::app);
			fil_time << my_size << " " << t_read_write_total << endl;
			fil_time.close();
...
double *ReadBlock (char *file_name){
	...
	int offset = sizeof(double) * (coord[1] * N + N * lenght * coord[0]) + 2*sizeof(int);
